...
async function dev(options) {
    ...
    if (initiateUpdateUrls) {
        const newURLs = generatePartnersURLs(exposedUrl, backendConfig?.configuration.authCallbackPath ?? frontendConfig?.configuration.authCallbackPath);
    ...
    const [previewableExtensions, nonPreviewableExtensions] = partition(localApp.extensions.ui, (ext) => ext.isPreviewable);
    ...
    if (nonPreviewableExtensions.length > 0) {
        ...
        additionalProcesses.push(devNonPreviewableExtensionTarget({
            app: localApp,
            apiKey,
            url: proxyUrl,
            token,
...
export function devNonPreviewableExtensionTarget({ extensions, app, url, apiKey, token, remoteExtensions, specifications, }) {
    return {
        prefix: 'extensions',
        action: async (stdout, stderr, signal) => {
...
async function validateCustomPorts(backendConfig, frontendConfig) {
